...
- **live limit bars**: how much of your 5-hour window and weekly caps you've burned and exactly when they reset, straight from the same endpoint the CLI's `/usage` screen uses. green → amber → red as you cook. fetched by a background child every 3 minutes, so the UI never waits on the network
