...
- **tab badges** (optional): perch can tint your actual windows terminal tabs — **red when a session needs you, green when it finished** — and the taskbar icon aggregates it, so a red flare reaches you even with the terminal minimized. no WT api exists for this; perch smuggles the ConEmu progress OSC (`ESC]9;4`) through claude code's own statusline, which windows terminal renders as a tab tint. wire it by setting your claude `statusLine` command to:
  ```
  cat > "$LOCALAPPDATA/AgentFocus/statusline.json"; sid=$(sed -n 's/.*"session_id" *: *"\([^"]*\)".*/\1/p' "$LOCALAPPDATA/AgentFocus/statusline.json" | head -1); cat "$LOCALAPPDATA/AgentFocus/statusline-text.txt" 2>/dev/null; [ -n "$sid" ] && cat "$LOCALAPPDATA/AgentFocus/badges/$sid.txt" 2>/dev/null; exit 0
  ```
  bonus: that same pipe feeds perch your official usage numbers (the statusline json) and lets perch show the limit bars in your statusline — three features, one snippet. sessions started before wiring keep their old look until restarted
