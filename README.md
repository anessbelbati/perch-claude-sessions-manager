...
- **compact mode**: double-click the header (or hit the `–` button) and perch folds into a tiny dynamic-island pill — the bird wearing your **5h limit as a colored ring**, next to colored need-you / working / done counts (`zzz` when everything sleeps). **click-driven, hover does nothing**: one click = the full card (the tooltip carries a passive counts + 5h% glance). **drag it and you carry only the bird** — capsule, ring and counts vanish and you're holding a scruff-grabbed birb with real dangle physics until you drop him and the pill reassembles
