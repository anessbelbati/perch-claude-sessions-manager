...
- **compact mode**: double-click the header (or hit the `–` button) and perch folds into a tiny dynamic-island pill — the bird wearing your **5h limit as a colored ring**, next to colored need-you / working / done counts (`zzz` when everything sleeps). hover it and the island crossfades open to show the chips + limit bars; **click it** and it expands back to the full card; **drag it** and it folds back to the pill in your hand (you always drag the pill, never the open island)
...
- **the bird is alive**: he's the center of the pill — a big bird whose limit-ring forms the capsule's left cap, everything else orbiting him. he perks up with an indignant wiggle when a session newly needs you, does a happy double-hop when work finishes, gives you a little bounce of a greeting when you hover, alternates head-tilts and pecks at the work while sessions run, does a grounding squash when you park the pill, and when everything's quiet he leans over asleep and **breathes** — slow, visible, alive. every motion is an event-driven render-only moment; the one loop (sleep-breathing) only runs when no session is doing anything
