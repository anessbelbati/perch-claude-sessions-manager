...
- **the bird is alive**: it perks up with an indignant wiggle when a session newly needs you, does a happy double-hop when work finishes, leans over asleep when everything's quiet, glances at the work every minute or so while sessions run, and does a little grounding squash when you park the pill. all event-driven render-only transforms — moments, not loops, so it costs nothing while idle
